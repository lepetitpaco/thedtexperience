...
<body>
    <div>
        ...
        <input type="text" id="name_search" placeholder="Search by name">
    </div>
    <div>
        <label for="country_search">Search by Country</label>
        <select id="country_search">
    ...
    <script type="text/javascript">
        ...
        var people = [];
        var names = ['Emma', 'Liam', 'Olivia', 'Noah', 'Ava', 'William', 'Isabella', 'James', 'Sophia', 'Oliver', 'Jean-Paul', 'Jean-Paul II'];
        var surnames = ['Smith', 'Johnson', 'Williams', 'Brown', 'Jones', 'Garcia', 'Miller', 'Davis', 'Rodriguez', 'Martinez'];
        var cities = ['New York', 'Los Angeles', 'Chicago', 'Houston', 'Phoenix', 'Philadelphia', 'San Antonio', 'San Diego', 'Dallas'];
        var countries = ['United States', 'Canada', 'Mexico', 'United Kingdom', 'Germany', 'France', 'Spain', 'Italy'];
        ...
        for (var i = 0; i < 200; i++) {
            var person = {
                ...
                birthday: (Math.floor(Math.random() * 12) + 1) + '/' + (Math.floor(Math.random() * 28) + 1) + '/' + (Math.floor(Math.random() * 100) + 1920),
        ...
        $(document).ready(function () { // BOF JQUERY
            $('#myTable').DataTable({
                paging: true,
                ordering: true,
                searching: true,
